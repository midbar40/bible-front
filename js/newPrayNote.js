...
const createPostIt = () => {
    ...
    <div class="scripture">
      <div class="scripture-1">
        <!-- <p class="scripture-1-text"></p> -->
      </div>
      <div class="scripture-2">
        <!-- <p class="scripture-2-text"></p> -->
      </div>
    </div>
    <div class="sermon">
...
const showSermonDetail = async (clickedSermonId) => {
    ...
    const sermonContent = document.querySelector('#sermon-content')
    const sermonTakeaway = document.querySelector('#sermon-takeaway')
    console.log('result.result.date :', result.result.date)
    if (
        sermonDate.value == '' &&
        sermonTitle.value == '' &&
        sermonScripture.value == '' && 
        ...
        previousSermonData.content = result.result.detail
        ...
        console.log('이전 데이터 :', previousSermonData.title, '인풋창 값 :', sermonTitle.value, '서버에서 가져온 값 :', result.result.title)
        const userResponse = confirm('작성중인 내용이 있습니다. 정말 취소하시겠습니까?')
        if (userResponse) {
            sermonDate.value = transformDateForSermon(result.result.date)
            sermonTitle.value = result.result.title
...
document.body.addEventListener('click', async function (e) {
    ...
    if (e.target.className == 'prayBucketTitle') {
        ...
        if (!postIt) {
            ...
            getPostItData()
                .then(data => {
                    const [post1, post2] = data
                    showPickPosts(post1, post2)
                })
            getSermon()
                .then(data => {
                    showSermon(data)
